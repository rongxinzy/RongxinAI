...
// Each explicit export starts from a clean directory so removed pages cannot
// survive as stale HTML from a previous build.
...
function extractSlideContent(filePath) {
  ...
  const match = content.match(/window\.slideDataMap\.set\(\s*\d+\s*,\s*`([\s\S]*?)`\s*,?\s*\);/);
  if (match) {
    return match[1].trim();
  }
  return null;
...
`,
  ...
  try {
    execSync(
      `npx tailwindcss -i "${tempCssInput}" -o "${tempCssOutput}" -c "${tempTailwindConfig}"`,
      {
        cwd: rootDir,
        stdio: 'pipe',
      },
...
function wrapWithSlidePage(bodyContent) {
  ...
  bodyContent = bodyContent.replace(/(<div)([\s>])/, '$1 class="slide-page"$2');
...
async function main() {
  ...
  slideFiles.forEach((slide, index) => {
    ...
    if (slide.number !== expectedNumber) {
      throw new Error(`幻灯片页码必须从 1 连续编号，缺少 slide-${expectedNumber}.js`);
    }
  });
  ...
  for (const slide of slideFiles) {
    ...
    if (content) {
      ...
      throw new Error(`无法从 ${slide.name} 提取 window.slideDataMap.set(...) 内容`);
    }
  }
