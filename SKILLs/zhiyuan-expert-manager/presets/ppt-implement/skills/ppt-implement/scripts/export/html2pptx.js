...
async function htmlToPptx(htmlFilePath, outputPptxPath, options = {}) {
    ...
    try {
        ...
        if (statistics.failed > 0) {
            const failedPages = statistics.failedSlides.map(slide => slide.index).join(', ');
            throw new Error(`幻灯片转换失败（页码: ${failedPages}）`);
        }
        if (pptx.slides.length !== slidePages.length) {
            throw new Error(`幻灯片数量不一致：预期 ${slidePages.length}，实际 ${pptx.slides.length}`);
        }
