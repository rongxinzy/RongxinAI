#!/usr/bin/env bash
# Render the first spreadsheet page to a real PNG for the controlled shortcut gate.
set -euo pipefail

if [ "$#" -ne 2 ]; then
  echo "Usage: $(basename "$0") <input.xlsx> <output.png>" >&2
  exit 2
fi
...
output="$2"
if [ ! -f "$input" ]; then
  echo "Spreadsheet input does not exist: $input" >&2
  exit 1
fi
if [[ "$output" != *.png ]]; then
...
if [ -z "$soffice_bin" ]; then
  echo "LibreOffice/soffice is required to render spreadsheet previews." >&2
  exit 1
fi
...
echo "Rendered XLSX preview: $output"
