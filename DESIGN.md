...
- 时长：普通交互 **100–250ms**；具备明确语义过程的动态图标 **300–450ms**，统一 `ease-out`（或 `transitionTimingFunction.smooth`）。超过 450ms 的动画需要理由。
...
| 语义动态图标 | 文件书写、对勾绘制、帽穗摆动、器件脉冲等可读的单次过程 | 仅图标内部 `opacity` / `transform`；一次完成、不循环 | 300–450ms |
...
7. **语义动态图标不等于普通 hover。** 只有动画本身表达明确过程时才可使用 300–450ms；鼠标悬停期间不重复播放，离开后复位。搜索、导航、普通按钮仍使用微交互档，不得借此整体放慢。
...
- [ ] 普通交互动效 ≤250ms；语义动态图标 300–450ms；只动 opacity/transform，幅度符合「动效语言」规范
