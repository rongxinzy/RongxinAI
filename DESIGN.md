...
全局系统提示唯一复用组件：`src/renderer/components/Toast.tsx`。任何新功能不得新建提示组件或自行拼接 Toast；跨页面提示必须派发 `app:showToast`，由 App 统一渲染。组件统一用 `src/renderer/services/toastNotification.ts` 的 `showAppToast` / `showAppErrorToast` / `showAppSuccessToast` 派发；禁止直接调用 sonner 的 `toast`——共享宿主当前没有挂载在任何位置，这类调用会被静默丢弃，用户什么也看不到（仓库里已有的直接调用属历史遗留，改到哪个文件就顺手迁哪个文件）。
...
- 系统提示统一复用 `src/renderer/components/Toast.tsx` 的视觉：内容自适应宽度、顶部居中；失败使用深色底配红色圆形 X，成功使用浅色底配绿色圆形对勾；信息提示直接用蓝色的 lucide `Info` 图标本身，不加浅色底盘——`Info` 自带圆环，叠底盘会形成同心双环；三档图标槽位同为 24px；不显示右侧关闭图标。
...
- 调用方显式声明的级别优先：`app:showToast` 传入的 `isError` / `isSuccess` 一旦给出，就不再按文案里的「失败」「错误」等字样升级为错误提示（`isSuccess: false` 同样是显式声明，代表中性汇报；判定顺序为 isError → isSuccess → 文案启发式）。批量诊断按结果分档：全部成功用成功提示，部分成功用信息提示（用户主动执行的那一步已经成功），一个都没成功才用错误提示——错误样式只留给没做成的操作。错误文案已经是成稿（i18n 文案或调用方已用 `normalizeError` 处理过）时用 `showAppErrorToast` 派发，不再二次套「操作失败：」；只有未加工的原始错误才交给归一化补前缀。
- 只读探测失败用中性信息提示（`showAppInfoToast`），不用错误样式：拉取/刷新模型列表这类请求失败后什么都没改动，列表保持原样，正文已经写明原因和下一步（检查 API Base URL、API Key 等），红色警报只会制造噪音。判定依据是「有没有改动状态」而不是「有没有失败」：用户已保存的配置整体不可用（批量连通性测试一个都没通过）仍然是错误提示，因为它需要用户立刻处理。
批量任务超过数秒且过程中没有页面内进度时，用同一条可续期的信息提示汇报进度（按钮触发的批量探测、测试等），结果出来后立即用结果汇报替换它；不得为进度新建提示容器或改用加载弹层。
