...
test('parseManagedSessionKey handles raw local session keys', () => {
  assert.deepEqual(parseManagedSessionKey('zhiyuan:abc-123'), {
    agentId: null,
    sessionId: 'abc-123',
  });
});
...
test('parseManagedSessionKey handles canonical local session keys', () => {
  assert.deepEqual(parseManagedSessionKey('agent:main:zhiyuan:abc-123'), {
    agentId: 'main',
    sessionId: 'abc-123',
  });
});
...
test('parseChannelSessionKey ignores managed local session keys', () => {
  assert.equal(parseChannelSessionKey('zhiyuan:abc-123'), null);
  assert.equal(parseChannelSessionKey('agent:main:zhiyuan:abc-123'), null);
});

test('channel sync does not treat managed local session keys as channel sessions', () => {
  const sync = createSync();

  assert.equal(isManagedSessionKey('agent:main:zhiyuan:abc-123'), true);
  assert.equal(sync.isChannelSessionKey('agent:main:zhiyuan:abc-123'), false);
  assert.equal(sync.resolveOrCreateSession('agent:main:zhiyuan:abc-123'), null);
  assert.equal(sync.resolveOrCreateMainAgentSession('agent:main:zhiyuan:abc-123'), null);
});
