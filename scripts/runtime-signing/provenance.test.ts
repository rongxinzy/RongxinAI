...
import { assertAncestor, executableHash, readBuildRun, readSigningRun, runId,
  RuntimeSources, signerThumbprint, SigningAuthority, validateManifest, validateRun } from './provenance.mjs';

const sha = 'a'.repeat(40);
const signer = 'B'.repeat(40);
const source = RuntimeSources.engram;
const build = { key: 'engram', sourceRepository: source.repo, sourceRunId: '123',
  sourceSha: sha, sourceTag: 'v1.20.0-zhiyuan.4' };
const run = { id: 123, repository: { full_name: source.repo },
  head_repository: { full_name: source.repo }, path: source.workflow,
  event: 'push', status: 'completed', conclusion: 'success', head_sha: sha,
  head_branch: build.sourceTag };
const manifest = { schemaVersion: 1, ...build, signingRepository: SigningAuthority.repo,
  signingRunId: '456', signerThumbprint: signer,
  files: source.files.map(name => ({ name, unsignedSha256: 'c'.repeat(64), signedSha256: 'd'.repeat(64) })) };
...
test('validates trusted completed runs and rejects every mismatched boundary', () => {
  ...
  for (const override of [
    { id: 124 }, { repository: { full_name: 'attacker/repo' } },
    { head_repository: { full_name: 'attacker/repo' } }, { path: 'other.yml' },
    { event: 'pull_request' }, { status: 'in_progress' }, { conclusion: 'failure' }, { head_sha: 'bad' },
  ]) expect(() => validateRun({ ...run, ...override }, source.repo, source.workflow, '123', 'push')).toThrow();
});

test('rejects malformed IDs, missing signer configuration and non-main ancestry', () => {
  for (const id of ['', '0', '-1', '1\n', '1/../../', '1e3', '9'.repeat(21)]) expect(() => runId(id)).toThrow();
  expect(runId('123')).toBe('123');
  expect(signerThumbprint('b '.repeat(40))).toBe(signer);
  for (const value of ['', 'b'.repeat(39), 'g'.repeat(40)]) expect(() => signerThumbprint(value)).toThrow();
  for (const status of ['behind', 'diverged', 'unknown']) expect(() => assertAncestor({ status })).toThrow();
  for (const status of ['ahead', 'identical']) expect(() => assertAncestor({ status })).not.toThrow();
});
...
test('peels annotated tags and rejects moved tags or unrelated main', async () => {
  ...
  for (const object of [{ type: 'commit', sha: 'f'.repeat(40) }, { type: 'tag', sha }]) {
    await expect(readBuildRun('engram', '123', async (repo: string, endpoint: string) =>
      endpoint.startsWith('git/') ? { object } : api(repo, endpoint))).rejects.toThrow();
  }
  await expect(readBuildRun('engram', '123', async (repo: string, endpoint: string) =>
    endpoint.startsWith('compare/') ? { status: 'diverged' } : api(repo, endpoint))).rejects.toThrow();
  await expect(readBuildRun('engram', '123', async () => ({ ...run, head_branch: 'main' }))).rejects.toThrow();
});

test('central signing must be a successful dispatch from the signing authority main', async () => {
  const signing = { ...run, id: 456, repository: { full_name: SigningAuthority.repo },
    head_repository: { full_name: SigningAuthority.repo }, path: SigningAuthority.workflow,
    event: 'workflow_dispatch', head_branch: 'main' };
  await expect(readSigningRun('456', async (_repo: string, endpoint: string) =>
    endpoint.startsWith('compare/') ? { status: 'identical' } : signing)).resolves.toEqual(signing);
  await expect(readSigningRun('456', async () => ({ ...signing, head_branch: 'feature' }))).rejects.toThrow();
});
...
test('rejects replayed, cross-runtime, wrong-signer or extra-file manifests', () => {
  ...
  for (const override of [
    { schemaVersion: 2 }, { key: 'sidecar' }, { sourceRepository: 'attacker/repo' },
    { sourceRunId: '124' }, { sourceSha: 'f'.repeat(40) }, { sourceTag: 'v1.20.0-zhiyuan.5' },
    { signingRepository: 'attacker/repo' }, { signingRunId: '457' }, { signerThumbprint: 'E'.repeat(40) },
    { files: [...manifest.files, manifest.files[0]] }, { files: [manifest.files[0], manifest.files[0]] },
...
test('workflow validates inputs before authenticating and signs only the manifest allowlist', () => {
  const workflow = readFileSync(new URL('../../.github/workflows/runtime-central-signing.yml', import.meta.url), 'utf8');
  expect(workflow.indexOf('manifest.mjs before')).toBeLessThan(workflow.indexOf('setup-certum-signing'));
...
test('manifest CLI rejects extra executables before signing and records both byte hashes', () => {
  ...
  const env = { ...process.env, RUNTIME_KEY: build.key, SOURCE_RUN_ID: build.sourceRunId,
    SOURCE_SHA: sha, SOURCE_TAG: build.sourceTag, GITHUB_RUN_ID: '456', CERTUM_CERT_THUMBPRINT: signer };
  ...
  expect(() => execFileSync(process.execPath,
    [script, 'before', directory], { env, stdio: 'pipe' })).toThrow();
});
