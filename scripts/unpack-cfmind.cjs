 ...
 * 通过 RongxinAI.exe (ELECTRON_RUN_AS_NODE=1 模式) 执行。
 *
 * 用法: RongxinAI.exe <本脚本路径> <tarPath> <destDir>
 *
 * 效果:
...
if (!tarPath || !destDir) {
  console.error('[unpack-cfmind] Usage: RongxinAI.exe unpack-cfmind.cjs <tarPath> <destDir>');
  process.exit(1);
}
