...
const translations: Record<LanguageType, Record<string, string>> = {
  zh: {
    ...
    localInferenceServiceConfigExternalWarning: '当前 Ollama 由外部进程启动，App 保存的服务启动参数不会影响这个正在运行的服务。',
    localInferenceServiceConfigExternalHint: '请先关闭外部 Ollama，再由 App 启动服务，参数才会生效。',
    localInferenceServiceConfigExternalNotApplied: '当前 Ollama 由外部进程运行，无法应用服务启动参数。请先关闭外部 Ollama，再由 App 启动服务。',
    ...
    localInferenceServiceConfigMaxLoadedHint: '同时驻留内存的模型数量；桌面单模型建议填 1，多模型切换可填 2，留空使用 Ollama 默认。',
    localInferenceServiceConfigNumParallelHint: '单个模型同时处理的请求数；桌面交互建议填 1，需要并发请求可填 2，过高会增加显存和内存压力。',
    ...
    marketplaceDataSourceHint: '来源：Ollama 官方库，离线时使用本地精选兜底',
    ...
    localInferenceLaunchLifecycleDescription: '这里配置本次模型加载方式。切换 GPU 预设时会重启 Ollama 服务，然后重新加载模型。',
    ...
    localInferenceLaunchGpuPresetDescription: '声明当前模型实例会让 Ollama 看到哪些 GPU。桌面工作站通常只运行一个模型，重启服务后再加载模型即可。',
    localInferenceLaunchGpuServiceDefault: '跟随服务设置',
    localInferenceLaunchGpuServiceDefaultHint: '不改动当前 Ollama 服务参数。',
    localInferenceLaunchGpuSingleAuto: '单卡优先',
    localInferenceLaunchGpuSingleAutoHint: '清空 GPU 限制，由 Ollama 自动选择，适合多数桌面场景。',
    localInferenceLaunchGpu0: '仅 GPU 0',
    localInferenceLaunchGpu0Hint: '只让 Ollama 使用第一张卡。',
    localInferenceLaunchGpu1: '仅 GPU 1',
    localInferenceLaunchGpu1Hint: '只让 Ollama 使用第二张卡。',
    ...
    localInferenceLaunchGpuCustomHint: '手动填写 CUDA_VISIBLE_DEVICES。',
    localInferenceLaunchGpuCustomValue: 'GPU 序号',
    localInferenceLaunchGpuCustomValueHint: '填写 CUDA_VISIBLE_DEVICES 格式，例如 0,1。',
    localInferenceLaunchGpuCurrent: '当前服务',
    localInferenceLaunchGpuWillUse: '本次启动',
    localInferenceLaunchGpuAutoVisible: 'Ollama 自动选择',
    localInferenceLaunchGpuKeepService: '保持当前服务设置',
    localInferenceLaunchGpuSpreadOn: '跨卡调度开启',
    localInferenceLaunchGpuSpreadOff: '跨卡调度关闭',
    localInferenceLaunchGpuRestartNotice: '该预设会重启 Ollama 服务，并卸载当前已加载模型。',
    localInferenceLaunchRestartFailed: 'Ollama 重启后未进入运行状态',
    ...
    localInferenceLaunchAccelerationHint: '自动表示交给 Ollama；仅 CPU 会设置 num_gpu=0。',
    localInferenceLaunchNumGpu: 'GPU 加速层数',
    localInferenceLaunchNumGpuHint: '写入 Ollama options.num_gpu；0 表示 CPU，留空使用 Ollama 默认。',
  ...
  en: {
    ...
    localInferenceServiceConfigExternalWarning: 'Ollama is currently started by an external process. Service startup parameters saved here will not affect that running service.',
    localInferenceServiceConfigExternalHint: 'Stop the external Ollama first, then start the service from this app for these parameters to apply.',
    localInferenceServiceConfigExternalNotApplied: 'Ollama is running as an external process, so service startup parameters cannot be applied. Stop the external Ollama first, then start it from this app.',
    ...
    localInferenceServiceConfigMaxLoadedHint: 'Number of models allowed to stay loaded at once. Use 1 for a desktop single-model setup, 2 for switching between models, or leave empty for Ollama default.',
    localInferenceServiceConfigNumParallelHint: 'Concurrent requests per model. Use 1 for desktop interaction, 2 for light concurrency. Higher values increase VRAM and memory pressure.',
    ...
    marketplaceDataSourceHint: 'Source: Ollama Library, with local curated fallback when offline',
    ...
    localInferenceLaunchLifecycleDescription: 'Configure how this model is loaded. Changing the GPU preset restarts Ollama and then reloads the model.',
    ...
    localInferenceLaunchGpuPresetDescription: 'Declare which GPUs Ollama can see for this model instance. A desktop workstation usually runs one model, so restarting the service before loading is acceptable.',
    localInferenceLaunchGpuServiceDefault: 'Follow service settings',
    localInferenceLaunchGpuServiceDefaultHint: 'Do not change the current Ollama service parameters.',
    localInferenceLaunchGpuSingleAuto: 'Single-GPU first',
    localInferenceLaunchGpuSingleAutoHint: 'Clear GPU restrictions and let Ollama choose automatically. Best for most desktops.',
    localInferenceLaunchGpu0: 'GPU 0 only',
    localInferenceLaunchGpu0Hint: 'Allow Ollama to use only the first GPU.',
    localInferenceLaunchGpu1: 'GPU 1 only',
    localInferenceLaunchGpu1Hint: 'Allow Ollama to use only the second GPU.',
    ...
    localInferenceLaunchGpuAutoVisible: 'Ollama auto-select',
    ...
    localInferenceLaunchGpuRestartNotice: 'This preset restarts Ollama and unloads currently loaded models.',
    localInferenceLaunchRestartFailed: 'Ollama did not enter the running state after restart',
    ...
    localInferenceLaunchAccelerationHint: 'Auto leaves this to Ollama. CPU only sets num_gpu=0.',
    localInferenceLaunchNumGpu: 'GPU Acceleration Layers',
    localInferenceLaunchNumGpuHint: 'Passed as Ollama options.num_gpu. 0 means CPU, empty uses Ollama default.',
