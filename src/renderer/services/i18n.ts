...
const translations: Record<LanguageType, Record<string, string>> = {
  zh: {
    ...
    localInferenceServiceConfigTitle: 'llama.cpp 参数',
    localInferenceServiceConfigDescription: '按服务、模型加载和请求生成语义整理当前可配置项；这些值保存后只在启动或重启服务时生效。',
    ...
    localInferenceServiceConfigDeviceHint: '限制 llama.cpp 可见的 GPU 序号，例如 0,1；留空使用自动选择。',
    ...
    marketplaceFilterSizeSmall: '轻量 < 4B',
    marketplaceFilterSizeDesktop: '桌面 4B-8B',
    marketplaceFilterSizeWorkstation: '工作站 8B-14B',
    marketplaceFilterSizeLarge: '大模型 > 14B',
    ...
    marketplaceSetupTitle: '需要启动 llama.cpp 服务',
    marketplaceSetupDescription: '请先启动 llama.cpp 服务后再安装模型',
    ...
    marketplaceNeedsRunning: '安装模型需要 llama.cpp 服务正在运行',
    marketplaceSetupOllamaAction: '{action} llama.cpp',
    localInferencePullTitle: '安装模型',
    localInferencePullHint: '输入 ModelScope 仓库名或 GGUF 模型标识，例如 unsloth/Qwen3.5-0.8B-GGUF。',
    ...
    localInferenceOptionThinkingForcedOpenLabel: '强制开启 thinking',
    localInferenceOptionThinkingForcedOpenHint: '仅对支持 thinking 的模板有效；普通模型保持自动。',
    ...
    localInferenceLaunchGpuPresetDescription: '声明当前模型实例会让 llama.cpp 看到哪些 GPU。桌面工作站通常只运行一个模型，重启服务后再加载模型即可。',
    ...
    localInferenceLaunchGpuSingleAutoHint: '清空 GPU 限制，由 llama.cpp 自动选择，适合多数桌面场景。',
    ...
    localInferenceLaunchGpuDualHint: '使用 GPU 0 和 GPU 1，并开启跨卡调度。',
    localInferenceLaunchGpuCustom: '自定义',
    localInferenceLaunchGpuCustomHint: '手动填写可见 GPU 序号。',
    localInferenceLaunchGpuCustomValue: 'GPU 序号',
    localInferenceLaunchGpuCustomValueHint: '填写 GPU 序号列表，例如 0,1。',
    ...
    localInferenceLaunchAccelerationHint: '自动表示交给 llama.cpp；仅 CPU 会设置 gpuLayers=0。',
    localInferenceLaunchNumGpu: 'GPU 加速层数',
    localInferenceLaunchNumGpuHint: '写入 llama.cpp 的 GPU 加速层数；0 表示 CPU，留空使用自动选择。',
    ...
    localInferenceLaunchAdvancedDescription: '这些参数会写入 llama.cpp 请求参数。不了解含义时保持默认即可。',
  ...
  en: {
    ...
    localInferenceServiceConfigTitle: 'llama.cpp Parameters',
    localInferenceServiceConfigDescription: 'Current configurable values are grouped by service, model loading, and request-generation semantics. Saved values apply only after starting or restarting the service.',
    ...
    localInferenceServiceConfigDeviceHint: 'Limit visible GPU indexes for llama.cpp, for example 0,1. Leave empty for automatic selection.',
    ...
    marketplaceOpenOllama: 'Model page',
    ...
    marketplaceFilterSizeSmall: 'Light < 4B',
    marketplaceFilterSizeDesktop: 'Desktop 4B-8B',
    marketplaceFilterSizeWorkstation: 'Workstation 8B-14B',
    marketplaceFilterSizeLarge: 'Large > 14B',
    ...
    marketplaceSetupTitle: 'llama.cpp Service Required',
    marketplaceSetupDescription: 'Start the llama.cpp service before installing models',
    ...
    marketplaceNeedsRunning: 'llama.cpp must be running to install models',
    marketplaceSetupOllamaAction: '{action} llama.cpp',
    localInferencePullTitle: 'Pull Model',
    localInferencePullHint: 'Enter a ModelScope repository or GGUF model ID, such as unsloth/Qwen3.5-0.8B-GGUF.',
    localInferencePullPlaceholder: 'unsloth/Qwen3.5-0.8B-GGUF',
    ...
    localInferenceOptionThinkingForcedOpenLabel: 'Force thinking',
    localInferenceOptionThinkingForcedOpenHint: 'Only effective for templates that support thinking. Keep auto for regular models.',
    ...
    localInferenceLaunchGpuPresetDescription: 'Declare which GPUs llama.cpp can use for this model instance. A desktop workstation usually runs one model, so restarting the service before loading is acceptable.',
    ...
    localInferenceLaunchGpuSingleAutoHint: 'Clear GPU restrictions and let llama.cpp choose automatically. Best for most desktops.',
    ...
    localInferenceLaunchGpuDualHint: 'Use GPU 0 and GPU 1 with cross-GPU scheduling enabled.',
    localInferenceLaunchGpuCustom: 'Custom',
    localInferenceLaunchGpuCustomHint: 'Manually set CUDA_VISIBLE_DEVICES.',
    localInferenceLaunchGpuCustomValue: 'GPU IDs',
    localInferenceLaunchGpuCustomValueHint: 'Use CUDA_VISIBLE_DEVICES format, for example 0,1.',
    ...
    localInferenceLaunchAccelerationHint: 'Auto leaves this to llama.cpp. CPU only sets gpuLayers=0.',
    localInferenceLaunchNumGpu: 'GPU Acceleration Layers',
    localInferenceLaunchNumGpuHint: 'Written as llama.cpp GPU offload layers. 0 means CPU, empty uses automatic selection.',
    ...
    localInferenceLaunchAdvancedDescription: 'These values are passed to llama.cpp request options. Keep defaults if unsure.',
