...
const translations: Record<LanguageType, Record<string, string>> = {
  zh: {
    ...
    localInferenceAccessAllowLanEnabledHint: '当前监听 0.0.0.0；局域网内其他设备可通过本机局域网 IP 访问。',
    localInferenceAccessAllowLanDisabledHint: '当前仅监听 127.0.0.1；只有本机可以访问。',
    ...
    localInferenceAccessRestartOnSaveHint: '当前 llama.cpp 正在运行。保存后会自动重启服务，并立即应用新的监听地址。重启过程中，当前已加载模型会按服务行为重新恢复。',
    localInferenceAccessRestartRequiredTitle: '修改后需要重启服务',
    localInferenceAccessRestartRequiredHint: '监听地址属于服务级配置。保存后，新配置会在下次启动或手动重启 llama.cpp 服务后生效。启用局域网访问后，其他设备应使用本机局域网 IP，而不是 0.0.0.0。',
    ...
    skillMarketplaceFeaturedTitle: 'ClawHub 热门技能',
    skillMarketplaceFeaturedDescription: '这里只展示一批热度较高的推荐技能；如需浏览全部技能，请前往 ClawHub。',
    skillMarketplaceOpenClawHub: '查看全部 ClawHub 技能',
  ...
  en: {
    ...
    localInferenceAccessAllowLanEnabledHint: 'The service now listens on 0.0.0.0. Other devices on the LAN can reach it through this machine\'s LAN IP.',
    localInferenceAccessAllowLanDisabledHint: 'The service now listens on 127.0.0.1. Only this machine can reach it.',
    ...
    localInferenceAccessRestartOnSaveHint: 'llama.cpp is currently running. Saving will automatically restart the service and apply the new listen address immediately. Loaded models will recover according to the service behavior during restart.',
    localInferenceAccessRestartRequiredTitle: 'Restart required',
    localInferenceAccessRestartRequiredHint: 'Listen address is a service-level setting. After saving, it takes effect the next time llama.cpp starts or after you manually restart the service. When LAN access is enabled, clients should use this machine\'s LAN IP instead of 0.0.0.0.',
    ...
    skillMarketplaceFeaturedTitle: 'Popular ClawHub Skills',
    skillMarketplaceFeaturedDescription:
      'This view shows a curated set of popular skills. Open ClawHub to browse the full catalog.',
    skillMarketplaceOpenClawHub: 'Browse all ClawHub skills',
