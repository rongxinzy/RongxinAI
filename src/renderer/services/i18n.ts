...
const translations: Record<LanguageType, Record<string, string>> = {
  zh: {
    ...
    localInferenceServiceConfigSaved: 'llama.cpp 服务参数已保存，下次启动生效。',
    localInferenceServiceConfigSavedRestartRequired: 'llama.cpp 服务参数已保存，重启服务后生效。',
    ...
    localInferenceLibrarySavedRestarted: '模型库目录已保存，llama.cpp 已重启，模型需要重新加载',
    ...
    localInferenceServiceConfigExternalWarning: '当前 llama.cpp 由外部进程启动，App 保存的服务启动参数不会影响这个正在运行的服务。',
    localInferenceServiceConfigExternalHint: '请先关闭外部 llama.cpp，再由 App 启动服务，参数才会生效。',
    localInferenceServiceConfigExternalNotApplied: '当前 llama.cpp 由外部进程运行，无法应用服务启动参数。请先关闭外部 llama.cpp，再由 App 启动服务。',
    ...
    localInferenceServiceConfigHostHint: 'llama.cpp 服务监听地址；桌面本机使用建议保留 127.0.0.1。',
    localInferenceServiceConfigPortHint: 'OpenAI-compatible API 端口；修改后客户端会使用新端口连接。',
    localInferenceAccessSettings: '访问设置',
    localInferenceAccessSettingsDescription: '配置 llama.cpp 的服务访问范围。多个模型仍通过同一个 OpenAI 兼容入口访问，并用 model 字段区分。',
    ...
    localInferenceAccessRestartOnSaveHint: '保存后立即生效。',
    ...
    localInferenceAccessSettingsSavedRestarted: '访问设置已保存，llama.cpp 已自动重启并生效。',
    localInferenceAccessSettingsSavedRestartRequired: '访问设置已保存；重启 llama.cpp 服务后生效。',
    ...
    localInferenceServiceConfigDeviceHint: '限制 llama.cpp 可见设备；请填写 0、1、2 这类设备索引，多个值用英文逗号分隔，保存时会映射成真实设备 ID。留空默认全部可见。',
    ...
    localInferenceServiceConfigCtxSizeHint: '服务默认上下文长度，对 OpenClaw 这类长提示词场景建议 8192 或 16384 起步。',
    localInferenceServiceConfigGpuLayersHint: 'GPU 卸载层数；auto 让 llama.cpp 自动判断，all 尝试全部卸载。',
    localInferenceServiceConfigBatchSizeHint: '提示词处理逻辑批大小；越大越快但更占显存和内存。',
    localInferenceServiceConfigUbatchSizeHint: '物理微批大小；显存紧张时降低。',
    localInferenceServiceConfigThreadsHint: '生成阶段 CPU 线程数；留空使用 llama.cpp 默认值。',
    localInferenceServiceConfigThreadsBatchHint: '提示词批处理阶段 CPU 线程数；留空使用 llama.cpp 默认值。',
    localInferenceServiceConfigTimeoutHint: 'llama.cpp 连接与加载超时时间，单位秒；模型较大或设备较慢时可适当调大。',
    ...
    localInferenceServiceConfigGpuUnavailableHint: '当前 llama.cpp 运行时未探测到可配置 GPU，仅检测到 CPU 或未返回加速设备。',
    localInferenceServiceConfigGpuDetectionFailedHint: '当前 llama.cpp 运行时未提供可用 GPU 枚举结果，无法配置显卡参数。',
    ...
    localInferenceServiceConfigMmapHint: '是否使用内存映射加载模型；关闭时会向 llama.cpp 传递 no-mmap。',
    ...
    localInferenceServiceConfigRestartHint: '当前服务正在运行，保存后需要重启 llama.cpp 才会应用。',
    localInferenceServiceConfigStartHint: '保存后下次启动 llama.cpp 时应用。',
    localInferenceInstall: '安装 llama.cpp',
    localInferenceImportRuntime: '从本地导入...',
    localInferenceImportRuntimeTooltip: '选择本地 llama.cpp backend 主包压缩包（zip 或 tar.gz），或选择已解压的完整目录，导入到 RongxinAI 管理的 backend 目录。',
    localInferenceImportRuntimeSuccess: 'llama.cpp 运行时导入成功。',
    localInferenceBackendManager: 'llama.cpp Backend',
    localInferenceBackendConfigTitle: '版本配置',
    localInferenceBackendConfigDescription: '管理 llama.cpp 的版本和 backend。服务参数仍保留在 llama.cpp 服务设置中。',
    localInferenceBackendListFailed: '无法读取 llama.cpp backend 列表。请刷新，或重启应用以加载最新 main/preload 代码。',
    localInferenceBackendListEmpty: '没有可用 backend。请确认 Gitee release 中存在 manifest.json，或使用内置 fallback 后重启应用。',
    localInferenceBackendHandlerMissing: '当前运行中的应用还没有加载新的 llama.cpp backend IPC。请完全退出应用后重新启动。',
    ...
    localInferenceBackendUpdated: 'llama.cpp backend 已更新。',
    localInferenceBackendUpdateFailed: 'llama.cpp backend 更新失败。',
    ...
    localInferenceImportGuideTitle: '导入 llama.cpp 运行时',
    localInferenceImportGuideDescription: '请确保所选目录包含完整的 llama.cpp 运行环境。以下是各操作系统的文件要求和推荐获取方式：',
    ...
    localInferenceImportGuideStep1LinkLabel: 'ggml-org/llama.cpp Releases',
    localInferenceImportGuideStep1Url: 'https://github.com/ggml-org/llama.cpp/releases',
    ...
    localInferenceImportGuideStep3: '点击「从本地导入...」，选择 llama.cpp backend 主包压缩包（zip 或 tar.gz），或选择已解压的完整目录',
    localInferenceImportGuideClose: '关闭',
    localInferenceImportGuideInlineDescription: '请先到 llama.cpp 官方发布页下载与你当前电脑匹配的 backend 主压缩包，再回到这里直接导入该压缩包。',
    localInferenceImportGuideInlineStepArchive: '1. 打开官方发布页',
    localInferenceImportGuideInlineReleaseUrl: 'https://github.com/ggml-org/llama.cpp/releases',
    ...
    localInferenceRuntimeReady: 'llama.cpp 已安装就绪。',
    localInferenceRuntimeMissing: 'llama.cpp 安装失败，请检查网络后重试。',
    localInferenceServiceNeedsBackendInstall: '请先在 llama.cpp 配置中安装 backend，然后再启动服务。',
    localInferenceServiceNeedsBackendSwitch: '请先在 llama.cpp 配置中切换到一个已安装 backend，然后再启动服务。',
    localInferenceRuntimeUninstall: '卸载 llama.cpp',
    localInferenceRuntimeUninstalled: 'App 管理的 llama.cpp 已卸载。',
    localInferenceRuntimeNotInstalled: '没有发现 App 管理的 llama.cpp。',
    localInferenceRuntimeUninstallFailed: 'llama.cpp 卸载失败。',
    ...
    localInferenceServiceStopped: 'llama.cpp 服务未运行，请先启动服务。',
    ...
    localInferenceModelLoading: '模型加载中',
    ...
    localInferenceModelLaunchLogWindowDescription: '展示 llama.cpp 模型启动阶段日志，用于确认启动进度和排查问题。',
    ...
    localInferenceUseOpenClawDisabledHint: '请先配置启动并加载模型，再用于 OpenClaw。',
    localInferenceSetOpenClawDone: '当前运行中的模型 {name} 已可供 OpenClaw 使用',
    localInferenceSetOpenClawFailed: '刷新 OpenClaw 可见模型失败',
    localInferenceSetOpenClawCtxTooSmall: '当前上下文长度 {ctx} 低于 OpenClaw 最低要求 ({min})，请先调大上下文长度再配置。',
    ...
    localInferenceLaunchLifecycleDescription: '这里配置本次模型加载方式。切换 GPU 预设时会重启 llama.cpp 服务，然后重新加载模型。',
    ...
    localInferenceLaunchGpuPresetDescription: '配置 llama.cpp 使用哪些 GPU，以及多卡时的拆分方式。',
    localInferenceLaunchGpuServiceDefault: '跟随服务设置',
    localInferenceLaunchGpuServiceDefaultHint: '不改动当前 llama.cpp 服务参数。',
    ...
    localInferenceLaunchGpu0Hint: '只让 llama.cpp 使用第一张卡。',
    localInferenceLaunchGpu1: '仅 GPU 1',
    localInferenceLaunchGpu1Hint: '只让 llama.cpp 使用第二张卡。',
    ...
    localInferenceLaunchGpuCustomHint: '手动指定 llama.cpp 使用哪些设备。',
    ...
    localInferenceLaunchGpuRestartNotice: '该预设会重启 llama.cpp 服务，并卸载当前已加载模型。',
    localInferenceLaunchRestartFailed: 'llama.cpp 重启后未进入运行状态',
    ...
    localInferenceLaunchAccelerationHint: '自动表示交给 llama.cpp；仅 CPU 会写入 n-gpu-layers=0。',
    localInferenceLaunchNumGpu: 'GPU 加速层数',
    localInferenceLaunchNumGpuHint: '写入 llama.cpp 的 n-gpu-layers；0 表示 CPU，留空使用自动选择。',
    ...
    localInferenceLaunchAdvancedDescription: '这些模型加载参数会写入 llama.cpp preset。不了解含义时保持默认即可。',
    localInferenceLaunchNumBatch: '批处理大小',
    localInferenceLaunchNumBatchHint: '提示词处理批大小；显存紧张时降低。',
    localInferenceLaunchUseMmap: '内存映射',
    localInferenceLaunchUseMmapHint: '写入 llama.cpp 的 mmap；禁用时等价于 no-mmap。',
    ...
    llamaCppProviderSettingsTitle: 'llama.cpp 仅作为本地运行态模型来源',
    llamaCppProviderSettingsDescription: '这里不维护 API Key 和模型列表。只有当前已加载到显存、并且服务可用的模型，才会出现在 Agent / OpenClaw 的模型选择中。',
    ...
    authQuotaExhausted:
      '今日免费额度已用完。您可以登录 RongxinAI Portal 购买套餐或积分包继续使用，或在设置中配置自己的 API Key。',
    authTopUpLink: '充值',
    authSettingsLink: '设置',
    ...
    coworkWorkingDirectoryHint: 'RongxinAI 将在此目录下执行命令',
    ...
    coworkEngineArchInfo: 'RongxinAI 采用双内核并行架构，两个引擎同时常驻运行，按职责固定分工：',
    ...
    coworkOpenClawErrorHint: '请检查网络连接或 OpenClaw 配置后重试。',
    ...
    coworkConfigSaveFailed: '保存 RongxinAI 配置失败，请稍后重试。',
    ...
    coworkPermissionDescription: 'RongxinAI 请求执行以下操作',
    coworkSelectionRequired: '请选择',
    coworkSelectionDescription: 'RongxinAI 需要你做出选择',
    coworkToolName: '工具名称',
    ...
    agentLlamaCppModelNotRunningHint: '该 llama.cpp 模型当前未运行，请先在本地推理页加载模型',
    agentLlamaCppModelNotRunningBlocked: '该 llama.cpp 模型当前未运行。请先到本地推理页加载模型，或改选其他模型。',
    agentLlamaCppContextInsufficientHint: '当前 llama.cpp 模型暂不可用于此任务，请先将上下文长度调到至少 32K（32768）后重新加载，或改用其他模型',
    agentLlamaCppContextUnknownHint: '?? llama.cpp ??????????????????? OpenClaw ? 32K?32768?????????????????????????????',
    agentLlamaCppContextTooSmallHint: '?? llama.cpp ??????????????????????? 32K?32768??????????????',
    agentLlamaCppTrainedContextTooSmallHint: '?? llama.cpp ???????????? 32K?32768?????????????????',
  ...
  en: {
    ...
    localInferenceServiceConfigTitle: 'llama.cpp Service Settings',
    ...
    localInferenceServiceConfigSaved: 'llama.cpp service parameters saved. They will apply on next start.',
    localInferenceServiceConfigSavedRestartRequired: 'llama.cpp service parameters saved. Restart llama.cpp to apply them.',
    ...
    localInferenceLibrarySavedRestarted: 'Model library directory saved. llama.cpp restarted; load a model again.',
    ...
    localInferenceServiceConfigExternalWarning: 'llama.cpp is currently started by an external process. Service startup parameters saved here will not affect that running service.',
    localInferenceServiceConfigExternalHint: 'Stop the external llama.cpp first, then start the service from this app for these parameters to apply.',
    localInferenceServiceConfigExternalNotApplied: 'llama.cpp is running as an external process, so service startup parameters cannot be applied. Stop the external llama.cpp first, then start it from this app.',
    ...
    localInferenceServiceConfigMmapLabel: 'mmap',
    localInferenceServiceConfigMlockLabel: 'mlock',
    localInferenceServiceConfigHostHint: 'llama.cpp listen address. Keep 127.0.0.1 for local desktop use.',
    ...
    localInferenceAccessSettingsDescription: 'Configure the llama.cpp service exposure scope. Multiple models still share one OpenAI-compatible endpoint and are distinguished by the model field.',
    ...
    localInferenceAccessRestartOnSaveHint: 'Takes effect immediately after saving.',
    ...
    localInferenceAccessSettingsSavedRestarted: 'Access settings saved. llama.cpp was restarted and the change is now active.',
    localInferenceAccessSettingsSavedRestartRequired: 'Access settings saved. Restart llama.cpp for the change to take effect.',
    ...
    localInferenceServiceConfigDeviceHint: 'Limit visible llama.cpp devices. Enter device indexes such as 0, 1, or 2. Separate multiple values with commas. The app maps them to real device IDs when saving. Leave empty to keep all devices visible.',
    ...
    localInferenceServiceConfigCtxSizeHint: 'Default context length. For OpenClaw-style long prompts, start with 8192 or 16384.',
    localInferenceServiceConfigGpuLayersHint: 'GPU offload layers. auto lets llama.cpp decide; all tries full offload.',
    ...
    localInferenceServiceConfigThreadsHint: 'CPU threads for generation. Leave empty for the llama.cpp default.',
    localInferenceServiceConfigThreadsBatchHint: 'CPU threads for prompt batch processing. Leave empty for the llama.cpp default.',
    localInferenceServiceConfigTimeoutHint: 'llama.cpp connection and load timeout in seconds. Increase it for larger models or slower devices.',
    ...
    localInferenceServiceConfigGpuUnavailableHint: 'The current llama.cpp runtime did not report any configurable GPU. Only CPU or no accelerator devices were detected.',
    localInferenceServiceConfigGpuDetectionFailedHint: 'The current llama.cpp runtime did not provide usable GPU enumeration results, so GPU parameters cannot be configured.',
    ...
    localInferenceServiceConfigMmapHint: 'Whether to load models with memory mapping. Disabled maps to llama.cpp no-mmap.',
    ...
    localInferenceServiceConfigRestartHint: 'llama.cpp is running. Restart the service to apply saved values.',
    localInferenceServiceConfigStartHint: 'Saved values apply the next time llama.cpp starts.',
    localInferenceInstall: 'Install llama.cpp',
    localInferenceImportRuntime: 'Import from local...',
    localInferenceImportRuntimeTooltip: 'Select a local llama.cpp backend archive (zip or tar.gz), or a fully extracted backend directory, to import into the RongxinAI-managed backend directory.',
    localInferenceImportRuntimeSuccess: 'llama.cpp runtime imported successfully.',
    localInferenceBackendManager: 'llama.cpp Backend',
    localInferenceBackendConfigTitle: 'llama.cpp Config',
    localInferenceBackendConfigDescription: 'Manage llama.cpp version and backend. Service parameters remain in llama.cpp service settings.',
    localInferenceBackendListFailed: 'Could not read the llama.cpp backend list. Refresh, or restart the app to load the latest main/preload code.',
    ...
    localInferenceBackendHandlerMissing: 'The running app has not loaded the new llama.cpp backend IPC yet. Fully quit and restart the app.',
    ...
    localInferenceBackendUpdated: 'llama.cpp backend updated.',
    localInferenceBackendUpdateFailed: 'Failed to update llama.cpp backend.',
    ...
    localInferenceImportGuideTitle: 'Import llama.cpp Runtime',
    localInferenceImportGuideDescription: 'The selected directory must contain a complete llama.cpp runtime environment. Here are the required files and recommended sources for each operating system:',
    ...
    localInferenceImportGuideStep3: 'Click "Import from local..." and select a llama.cpp backend archive (zip or tar.gz), or a fully extracted backend directory',
    localInferenceImportGuideClose: 'Close',
    localInferenceImportGuideInlineDescription: 'First download the backend main archive that matches your current machine from the official llama.cpp releases page, then import that archive directly here.',
    ...
    localInferenceRuntimeReady: 'llama.cpp is installed and ready.',
    localInferenceRuntimeMissing: 'Failed to install llama.cpp. Check the network and try again.',
    localInferenceServiceNeedsBackendInstall: 'Install a backend in llama.cpp Config before starting the service.',
    localInferenceServiceNeedsBackendSwitch: 'Switch to an installed backend in llama.cpp Config before starting the service.',
    localInferenceRuntimeUninstall: 'Uninstall llama.cpp',
    localInferenceRuntimeUninstalled: 'The app-managed llama.cpp has been uninstalled.',
    localInferenceRuntimeNotInstalled: 'No app-managed llama.cpp was found.',
    localInferenceRuntimeUninstallFailed: 'Failed to uninstall llama.cpp.',
    ...
    localInferenceServiceStopped: 'llama.cpp is not running. Start the service first.',
    ...
    localInferenceModelLoading: 'Model loading',
    ...
    localInferenceModelLaunchLogWindowDescription: 'Shows llama.cpp model startup logs so you can track progress and troubleshoot startup issues.',
    ...
    localInferenceUseOpenClawDisabledHint: 'Configure and load the model before using it for OpenClaw.',
    localInferenceSetOpenClawDone: 'Running model {name} is now visible to OpenClaw',
    localInferenceSetOpenClawFailed: 'Failed to refresh OpenClaw-visible local models',
    localInferenceSetOpenClawCtxTooSmall: 'Current context length {ctx} is below the minimum required for OpenClaw ({min}). Please increase the context length first.',
    ...
    localInferenceLaunchLifecycleDescription: 'Configure how this model is loaded. Changing the GPU preset restarts llama.cpp and then reloads the model.',
    ...
    localInferenceLaunchGpuPresetDescription: 'Choose which GPUs llama.cpp can use and how multi-GPU splitting works.',
    localInferenceLaunchGpuServiceDefault: 'Follow service settings',
    localInferenceLaunchGpuServiceDefaultHint: 'Do not change the current llama.cpp service parameters.',
    localInferenceLaunchGpuSingleAuto: 'GPU 0 only',
    localInferenceLaunchGpuSingleAutoHint: 'Pin llama.cpp to CUDA0 and disable multi-GPU splitting.',
    localInferenceLaunchGpu0: 'GPU 0 only',
    localInferenceLaunchGpu0Hint: 'Allow llama.cpp to use only the first GPU.',
    localInferenceLaunchGpu1: 'GPU 1 only',
    localInferenceLaunchGpu1Hint: 'Allow llama.cpp to use only the second GPU.',
    localInferenceLaunchGpuDual: 'Use both GPUs',
    localInferenceLaunchGpuDualHint: 'Pin llama.cpp to CUDA0 and CUDA1, then split layers across both GPUs.',
    localInferenceLaunchGpuCustom: 'Custom',
    localInferenceLaunchGpuCustomHint: 'Manually choose which devices llama.cpp can use.',
    ...
    localInferenceLaunchGpuRestartNotice: 'This preset restarts llama.cpp and unloads currently loaded models.',
    localInferenceLaunchRestartFailed: 'llama.cpp did not enter the running state after restart',
    ...
    localInferenceLaunchAccelerationHint: 'Auto leaves this to llama.cpp. CPU only writes n-gpu-layers=0.',
    localInferenceLaunchNumGpu: 'GPU Acceleration Layers',
    localInferenceLaunchNumGpuHint: 'Written as llama.cpp n-gpu-layers. 0 means CPU, empty uses automatic selection.',
    ...
    localInferenceLaunchAdvancedDescription: 'These model loading parameters are written to the llama.cpp preset. Keep defaults if unsure.',
    ...
    localInferenceLaunchUseMmapHint: 'Written as llama.cpp mmap. Disabled is equivalent to no-mmap.',
    ...
    llamaCppProviderSettingsTitle: 'llama.cpp is a local runtime model source',
    llamaCppProviderSettingsDescription: 'API keys and model lists are not managed here. Only models that are currently loaded into VRAM and available from the service appear in Agent / OpenClaw model pickers.',
    ...
    copied: 'Copied',
    copyrightHolder: 'RongxinAI. All rights reserved.',
    ...
    authQuotaExhausted:
      'Daily free quota exhausted. Visit RongxinAI Portal to purchase a plan or credits, or configure your own API Key in Settings.',
    ...
    // RongxinAI
    cowork: 'RongxinAI',
    coworkSettings: 'RongxinAI Settings',
    ...
    coworkWorkingDirectoryHint: 'RongxinAI will execute commands in this directory',
    ...
    coworkEngineArchInfo: 'RongxinAI runs a dual-kernel architecture with both engines running simultaneously, each with fixed responsibilities:',
    ...
    coworkOpenClawErrorHint: 'Check your network connection or OpenClaw configuration and try again.',
    ...
    coworkConfigSaveFailed: 'Failed to save RongxinAI settings. Please try again.',
    ...
    coworkPermissionDescription: 'RongxinAI is requesting to perform the following action',
    coworkSelectionRequired: 'Please Choose',
    coworkSelectionDescription: 'RongxinAI needs your input',
    ...
    coworkExecutionSteps: 'Execution steps',
    ...
    agentLlamaCppModelNotRunningHint: 'This llama.cpp model is not running. Load it from Local Inference first.',
    agentLlamaCppModelNotRunningBlocked: 'This llama.cpp model is not running. Load it from Local Inference first or choose another model.',
    agentLlamaCppContextInsufficientHint: 'This llama.cpp model is currently unavailable for this task. Reload it with a context window of at least 32K (32768), or switch to another model.',
    agentLlamaCppContextUnknownHint: 'This llama.cpp model did not report its context window, so OpenClaw cannot verify the 32K (32768) requirement. Reload the model with an explicit context size, or switch to another model.',
    agentLlamaCppContextTooSmallHint: 'This llama.cpp model is currently unavailable for this task. Reload it with a context window of at least 32K (32768), or switch to another model.',
    agentLlamaCppTrainedContextTooSmallHint: 'This llama.cpp model was trained with a context window below 32K (32768), so it cannot be used for this task. Switch to another model.',
    ...
    skillMarketplaceFeaturedTitle: 'Featured ModelScope Skills',
    skillMarketplaceFeaturedDescription:
      'This view shows a curated set of installable skills. Open ModelScope to browse the full catalog.',
