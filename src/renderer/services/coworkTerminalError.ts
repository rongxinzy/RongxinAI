...
/** Prefer extracting error.message from JSON when present. */
...
/**
 * User-facing error text: use error.message directly when present;
 * do not wrap with a request-failed prefix.
 */
export const extractUserFacingErrorMessage = (raw: string): string => {
  ...
  // Support JSON embedded after a prefix, e.g. "xxx: {...}"
...
export const isCoworkTerminalErrorMessage = (message: CoworkMessage): boolean => {
  ...
  // Legacy sessions store error JSON in content; treat as terminal so it is not folded under "Working"
...
/** Persist Direct Chat failures as canonical terminal errors for TurnBlock. */
export const createDirectChatErrorMessage = (
  error: unknown,
  timestamp = Date.now(),
): CoworkMessage => {
...
/** Terminal error bubble text; prefer metadata.error and unwrap JSON payloads. */
