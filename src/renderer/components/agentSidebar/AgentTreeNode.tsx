...
const AgentTreeNode: React.FC<AgentTreeNodeProps> = ({
  ...
  return (
    <div className="space-y-0.5">
      <div
        className={`group sticky top-10 ${menuOpen ? 'z-50' : 'z-20'} ml-[-6px] h-7 w-[calc(100%+12px)] bg-surface-raised`}
      >
        <Button
          variant="ghost"
        ...
        <div
          className={`absolute right-1.5 top-1/2 flex -translate-y-1/2 items-center gap-0.5 transition-opacity ${menuOpen ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
        >
          <DropdownMenu open={menuOpen} onOpenChange={setMenuOpen}>
            <DropdownMenuTrigger
              render={
                <Button
                  variant="ghost"
                  size="icon-xs"
                  className="opacity-[0.3] hover:opacity-[0.46]"
            ...
            <DropdownMenuContent align="end" className="min-w-[104px]">
              <DropdownMenuItem onClick={handleEditAgent}>
                <Pencil className="h-3.5 w-3.5" /> {i18nService.t('edit')}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={handleToggleAgentPin}>
      ...
      {shouldRenderTasks && (
        <div
          className={`grid w-full min-w-0 max-w-full transition-all duration-200 ease-out motion-reduce:transition-none ${isTaskGroupVisible ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}
        >
          <div
            className={`min-h-0 min-w-0 max-w-full ${isTaskGroupVisible && !isTaskGroupTransitioning ? 'overflow-visible' : 'overflow-hidden'} ${isTaskGroupVisible ? '' : 'pointer-events-none'}`}
            role="group"
            ...
            <div className="min-w-0 max-w-full space-y-0.5">
              ...
              {agent.tasks.map(task => (
                <AgentTaskRow
                  ...
                  onTogglePin={p => onToggleTaskPin(task, p)}
                  onRename={t => onRenameTask(task, t)}
