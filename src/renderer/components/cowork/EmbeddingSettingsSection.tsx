...
const EmbeddingSettingsSection: React.FC<EmbeddingSettingsSectionProps> = (props) => {
  const {
    embeddingEnabled, embeddingProvider, embeddingModel, embeddingVectorWeight,
    embeddingRemoteBaseUrl, embeddingRemoteApiKey,
    onEmbeddingEnabledChange, onEmbeddingProviderChange, onEmbeddingModelChange,
    onEmbeddingVectorWeightChange, onEmbeddingRemoteBaseUrlChange, onEmbeddingRemoteApiKeyChange,
  } = props;
        ...
        {embeddingEnabled && (
          <div className="space-y-3 pt-2">
            <div>
              ...
              <Select value={embeddingProvider} onValueChange={(v) => onEmbeddingProviderChange(v ?? 'openai')}>
                <SelectTrigger className="w-full mt-1"><SelectValue /></SelectTrigger>
                <SelectContent>
                  {PROVIDERS.map((p) => (
                    <SelectItem key={p.value} value={p.value}>{i18nService.t(p.label)}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <Collapsible open={showAdvanced} onOpenChange={setShowAdvanced}>
              <CollapsibleTrigger>
                <Button variant="link" size="sm" className="text-xs h-auto p-0">{showAdvanced
                  ? i18nService.t('coworkMemoryAdvancedHide')
                  : i18nService.t('coworkMemoryAdvancedShow')}</Button>
              </CollapsibleTrigger>
