...
export class ChannelAccountManager {
  ...
  getStatus(
    ...
    return {
      dingtalk: {
        instances: common(config.dingtalk.instances, item => !!(item.clientId && item.clientSecret)),
      },
      feishu: {
        instances: common(config.feishu.instances, item => !!(item.appId && item.appSecret)).map(
          item => ({ ...item, botOpenId: null as string | null, error: null as string | null }),
        ),
      },
      telegram: {
      ...
      wecom: {
        instances: common(config.wecom.instances, item => !!(item.botId && item.secret)).map(item => ({
          ...item,
          botId: config.wecom.instances.find(source => source.instanceId === item.instanceId)?.botId || null,
        })),
      },
      weixin: {
  ...
  private async probe(platform: Platform, config: IMGatewayConfig): Promise<string> {
    if (platform === 'telegram') {
      const instance = config.telegram.instances.find(item => item.enabled) ?? config.telegram.instances[0];
      ...
      const instance = config.discord.instances.find(item => item.enabled) ?? config.discord.instances[0];
      if (!instance?.botToken) throw new Error('Bot token is required.');
      await fetchJsonWithTimeout('https://discord.com/api/v10/users/@me', {
        headers: { Authorization: `Bot ${instance.botToken}` },
      }, CONNECTIVITY_TIMEOUT_MS);
    } else if (platform === 'feishu') {
      const instance = config.feishu.instances.find(item => item.enabled) ?? config.feishu.instances[0];
      ...
      const instance = config.dingtalk.instances.find(item => item.enabled) ?? config.dingtalk.instances[0];
      if (!instance?.clientId || !instance.clientSecret) throw new Error('App credentials are required.');
      ...
      if (!instance?.appId || !instance.appSecret) throw new Error('App credentials are required.');
      await fetchJsonWithTimeout('https://bots.qq.com/app/getAppAccessToken', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ appId: instance.appId, clientSecret: instance.appSecret }),
      }, CONNECTIVITY_TIMEOUT_MS);
    } else if (platform === 'wecom') {
      const instance = config.wecom.instances.find(item => item.enabled) ?? config.wecom.instances[0];
  ...
  async verifyFeishuCredentials(appId: string, appSecret: string): Promise<{ success: boolean; error?: string }> {
    try {
  ...
  async verifyDingTalkCredentials(clientId: string, clientSecret: string): Promise<{ success: boolean; error?: string }> {
    try {
  ...
  async startDingTalkInstallQrcode(): Promise<{ url: string; deviceCode: string; interval: number; expireIn: number }> {
    ...
    const init = await fetchJsonWithTimeout<{ errcode: number; errmsg?: string; nonce?: string }>(
      ...
      { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ source: 'DING_DWS_CLAW' }) },
      CONNECTIVITY_TIMEOUT_MS,
    );
    if (init.errcode !== 0 || !init.nonce) throw new Error(init.errmsg || 'DingTalk registration init failed.');
    const result = await fetchJsonWithTimeout<{ errcode: number; errmsg?: string; device_code?: string; verification_uri_complete?: string; interval?: number; expires_in?: number }>(
      `${baseUrl}/app/registration/begin`,
      { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ nonce: init.nonce }) },
      CONNECTIVITY_TIMEOUT_MS,
    );
    if (result.errcode !== 0 || !result.device_code || !result.verification_uri_complete) throw new Error(result.errmsg || 'DingTalk registration failed.');
    return { url: result.verification_uri_complete, deviceCode: result.device_code, interval: result.interval ?? 5, expireIn: result.expires_in ?? 600 };
  }

  async pollDingTalkInstall(deviceCode: string): Promise<{ done: boolean; clientId?: string; clientSecret?: string; error?: string }> {
    const result = await fetchJsonWithTimeout<{ errcode: number; errmsg?: string; status?: string; client_id?: string; client_secret?: string; fail_reason?: string }>(
    ...
    if (result.status === 'SUCCESS' && result.client_id && result.client_secret) return { done: true, clientId: result.client_id, clientSecret: result.client_secret };
    if (result.status === 'FAILED') return { done: false, error: result.fail_reason || result.errmsg };
    return { done: false };
  }
}
