...
test('installModel reports download speed in progress events', async () => {
  const modelsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'llamacpp-install-speed-'));
  const manager = new LlamaCppManager(() => ({ modelsDir }));
  manager.refreshModelsAfterInstall = async () => undefined as any;

  const originalFetch = global.fetch;
  const originalDateNow = Date.now;
  const progressEvents: LlamaCppInstallProgress[] = [];
  let now = 1_000;
  ...
  try {
    ...
    await manager.installModel(
      {
        ...
        filePath: 'Qwen3.5-0.8B-Q4_0.gguf',
    ...
    const downloadProgressEvents = progressEvents.filter(
      progress => progress.phase === 'downloading-progress',
    );
    expect(downloadProgressEvents.length).toBeGreaterThan(0);
    expect(downloadProgressEvents.some(progress => typeof progress.speed === 'number' && progress.speed > 0)).toBe(true);
  } finally {
    global.fetch = originalFetch;
