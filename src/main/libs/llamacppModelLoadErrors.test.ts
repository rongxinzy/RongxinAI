...
describe('llamacppModelLoadErrors', () => {
  ...
  test('classifies GPU absence and GPU probe errors separately', () => {
    ...
    )).toBe(LlamaCppModelLoadFailureReason.GpuNotFound);

    expect(classifyLlamaCppModelLoadError(
      'nvidia-smi timed out while querying devices',
    )).toBe(LlamaCppModelLoadFailureReason.GpuProbeFailed);
  });

  test('classifies context, model file, service, and timeout errors', () => {
    expect(classifyLlamaCppModelLoadError(
      'requested ctx-size exceeds supported context window',
    )).toBe(LlamaCppModelLoadFailureReason.ContextTooLarge);

    expect(classifyLlamaCppModelLoadError(
      'invalid GGUF magic while reading model file',
    )).toBe(LlamaCppModelLoadFailureReason.ModelFileInvalid);

    expect(classifyLlamaCppModelLoadError(
      'fetch failed: ECONNREFUSED 127.0.0.1:8080',
    )).toBe(LlamaCppModelLoadFailureReason.ServiceUnavailable);

    expect(classifyLlamaCppModelLoadError(
      'llama.cpp model qwen did not become ready before timeout',
    )).toBe(LlamaCppModelLoadFailureReason.StartupTimeout);
  });

  test('classifies model limit and model-not-found errors', () => {
    expect(classifyLlamaCppModelLoadError(
      'loaded model limit reached',
    )).toBe(LlamaCppModelLoadFailureReason.ModelsLimitReached);

    expect(classifyLlamaCppModelLoadError(
      'model file does not exist: C:/models/missing.gguf',
    )).toBe(LlamaCppModelLoadFailureReason.ModelNotFound);
  });

  test('marks only resource and transient startup failures retryable', () => {
    expect(isRetryableLlamaCppModelLoadError(
    ...
    expect(isRetryableLlamaCppModelLoadError(
      LlamaCppModelLoadFailureReason.ContextTooLarge,
    )).toBe(true);
    expect(isRetryableLlamaCppModelLoadError(
      LlamaCppModelLoadFailureReason.ServiceUnavailable,
    )).toBe(true);
    expect(isRetryableLlamaCppModelLoadError(
      LlamaCppModelLoadFailureReason.StartupTimeout,
    )).toBe(true);
    ...
    expect(isRetryableLlamaCppModelLoadError(
      LlamaCppModelLoadFailureReason.ModelNotFound,
    )).toBe(false);
  });

  test('maps failure reasons to main-process i18n keys', () => {
    expect(getLlamaCppModelLoadFailureI18nKey(
      LlamaCppModelLoadFailureReason.GpuNotFound,
    )).toBe('llamacppLoadModelGpuNotFound');
    expect(getLlamaCppModelLoadFailureI18nKey(
      LlamaCppModelLoadFailureReason.Unknown,
    )).toBe('llamacppLoadModelUnknown');
  });
});
