...
test('MarketplaceService sorts featured models first for empty queries and applies filters', () => {
  ...
  expect(
    result.every(item => item.tags.includes('reasoning') || item.capability === 'reasoning'),
  ).toBe(true);
...
test('MarketplaceService retries rate limits before rotating to the next token', async () => {
  ...
  const fetchMock = vi.fn(async (url: string, init?: RequestInit) => {
    ...
    if (
      (init?.headers as Record<string, string> | undefined)?.Authorization === 'Bearer first-token'
    ) {
