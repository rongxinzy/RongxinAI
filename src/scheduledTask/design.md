...
`project` 必须匹配任务的 `delivery.accountId`（默认 `default`）。Runtime 先校验账号与 `scheduleVersion`，再以 `(taskId, scheduleVersion, scheduledAt)` 原子 claim Run；重复、过期或跨账号 trigger 不会执行 Pi。
