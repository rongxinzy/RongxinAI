...
test('runs a canonical task using the embedded Pi runtime and waits for complete', async () => {
  ...
  const coworkStore = { getConfig: () => ({ workingDirectory: process.cwd(), systemPrompt: '', executionMode: 'local' }), createSession: () => session, getSession: () => null };
  ...
  await expect(new PiScheduledTaskExecutor(runtime as never, coworkStore as never).execute(task, run)).resolves.toEqual({ sessionId: 'cowork-1' });
});
