...
An OpenAI-compatible model may declare validated `reasoningCompatibility` metadata. API v1 currently accepts the DeepSeek wire format only and requires both reasoning-effort support and assistant `reasoning_content` replay for continued tool-call conversations. The host converts this generic descriptor into internal runtime options; enterprise extensions do not import or depend on the application's runtime implementation.
